 ...
 * 空间进化配方整段作废):
 *   **拥有 3 把相同的武器 → 当场自动合成为 1 把更强的合成武器**(占 1 个武器槽)。
 * 合成是**获得第 3 把的那一刻**触发的(用户设计会:不是船坞手动操作,是即时合成),
 * 合成结果不可逆、不能再拆回 3 把 —— 与旧"进化不可逆"同一条口径。
 *
 * 本表只描述"3×哪把基础武器 → 哪把合成武器";触发流程住在 sim/world.ts(槽位记账 +
 * 获得第 3 把当场兑换)。合成结果与基础武器一一对应(6 条线),顺序与 towers.ts 里
 * TOWER_STORM_CANNON=6..TOWER_THORN=11 的编号一致(merges.test 钉着这条对应,错位 = 配方张冠李戴)。
 *
 * 旧的"满级塔 + 相邻支援"进化(evolutions.ts / isEvolutionTower)随甲板网格一起删除:
 * 合成不再要求等级/邻接,只数同型武器的把数 —— 这是"槽位制"对"网格制"的替换。
 */
import {
  TOWER_ANNIHILATION,
  TOWER_ARC,
...
/**
 * 配方查询:这把武器能不能参与合成(是某条合成线的基础)。没有配方返回 -1 ——
 * 与 DeckCell.towerType 的"没有"是同一条表达(World 获得第 3 把时靠它查合成结果)。
 */
 ...
 * 能在三选一/商店里买到就违背了"合成只在获得第 3 把时触发"的闸门。

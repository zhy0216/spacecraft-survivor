...
/**
 * 一张法令卡的星币价。口径:约等于**第三个航段(两分钟)的整段击杀收入**(3% 掉率下
 * 段3 期望 ≈ 970 面值 × 3% ≈ 29 枚,见 STARCOIN_DROP_CHANCE 的重锚账);段1/段2 的
 * 段收入只有它的 1/4~1/2 —— 买法令是"押上一两个航段的收入换永久被动"的取舍,不是顺手的小费。
...
/**
 * 一把武器卡的星币价。口径:约等于**第三到第四个航段**的击杀收入(3% 掉率下段3 ≈ 29、
 * 段4 ≈ 48,见 STARCOIN_DROP_CHANCE 的重锚账),前两个航段要攒着才买得起 ——
 * 30 的定位是"攒两段买一把新武器":武器获得的主要通道是商店,
 * 价定低了,升级三选一里那 40% 的新武器(几乎白给)就会反过来抢商店的生意。
 */
 ...
 *
 * 概率原定 10%,但真脚本实测一局约 1958 杀、星币收入 **269 枚**(economy.test 的
 * 真脚本长跑同款口径),按价 25-30 一件能买 6-14 件 —— 设计注释写的"整局大概只买得起
 * 一样东西"差了**一个数量级**,重摇(10)在 269 收入下几乎免费、付费修复买穿
 * "船体受伤 = 永久债务"的张力、星图协议"押满翻五倍"锚在过量的基线上。
 * 重锚到 **3%**:同脚本收入 ≈ 80-100 枚(含开局白送),一局 3-4 次商店决策 ——
 * 四家信标店每家一件,店与店之间要攒,首店配白送恰好一件(见 STARTING_STAR_COINS)。
 *
 * 这条概率是**星图协议**(data/edicts 的 EDICT_STARCHART,+2 个点/层,5 层 = 12.5%)
 * 的作用锚点:基础 3% × 押满 12.5% ≈ 翻五倍 —— 星币稀缺 ⇒ 抬概率的那条法令
 * 从"顺手拿"变成一条真正的 build 路线(翻倍锚与原设计一致)。
 *
 ...
 */
export const STARCOIN_DROP_CHANCE = 0.03;

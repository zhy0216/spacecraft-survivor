 ...
 * 段3 期望 ≈ 970 面值 × 3% ≈ 29 枚);掉率抬到 23% 后段3 期望 ≈ 223 枚,25 只是它的零头 ——
 ...
 * 段4 ≈ 48),前两个航段要攒着才买得起;掉率抬到 23% 后段3/段4 ≈ 220/330,30 是零头 ——
...
/**
 * —— 星币掉落概率(用户设计会 + 二轮审查重锚 → 用户设计会再拍 +20 个百分点)——
 ...
 * 同脚本(1958 杀)期望收入 ≈ 600-700 枚,25-30 的商店价随手可买,重摇(10)与付费修复(25)
 * 的"要不要花"张力弱化,星图协议押满(+10 个点 = 33%)相对增量只剩约 +43%。
 * 商店价 / 白送 / 星图协议的取舍随这条口径留到下一次重锚(占位待调),
 * 真裁判(sim/economy.test 的固定 seed 长跑)按新掉率重钉收入窗口。
 *
 * 这条概率是**星图协议**(data/edicts 的 EDICT_STARCHART,+2 个点/层,5 层 = 33%)
 * 的作用锚点:基础 23% 已宽裕,抬概率的法令从"一条 build 路线"降回"锦上添花"。
 *
 ...
 */
export const STARCOIN_DROP_CHANCE = 0.23;
 ...
 * 掉率抬到 23% 后段1 期望 ≈ 37 枚,首店 ≈ 62 枚够两件 —— 白送本身不动(占位待调),

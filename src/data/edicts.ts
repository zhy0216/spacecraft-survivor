 ...
 *   乘法档一律 **base^level**(2 层散热协议 = ×1.5² = ×2.25,连乘永远推不到 ≤ 0);
...
export const EDICTS: EdictDef[] = [
  ...
  },
  {
    ...
    fireRateMul: 1,
    reloadMul: 1,
    heatMaxMul: 1.5, // 原散热器:过热上限 +50%(5 层 ≈ ×7.6 —— 实际上就是"再也不过热")
    chargeRateMul: 1,
    damageMul: 1,
    hullHpAdd: 0,
    damageTakenMul: 1,
  ...
  },
  {
    ...
    // 这条是唯一能抬那个概率的东西。+8 个点/层 ⇒ 5 层 = 50%,正好是"押满这条轴 = 收入翻五倍"
    // 的整档;概率的上夹在 sim/edictBuffs.ts(总概率恒落在 [0,1])
    throttle: EDICT_THR_NONE,
    fireRateMul: 1,
    reloadMul: 1,
    heatMaxMul: 1,
    ...
    turnRateAdd: 0,
    cruiseSpeedMul: 1,
    starCoinChanceAdd: 0.08, // 星币掉落概率 +8 个百分点(占位待调)

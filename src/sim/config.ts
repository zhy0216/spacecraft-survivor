...
export const tuning = {
  ...
  // 大一圈):战斗中船是一枚与虫群同台的棋子,甲板细节只在升级时停 / Tab 甲板视图里放大看
  // (镜头的世界→屏幕缩放由下面 cameraShipHeightFraction 配平,敌人观感大小不变)。
  shipLength: 48, // 船体长度(世界 px,船头→船尾);÷ 4 行 = 12
  shipWidth: 36, // 船体宽度(世界 px,船宽方向);÷ 3 列 = 12 —— 两轴同档,即实际格边长
  ...
  // 240 是"航线擦过去 = 收得下"的宽容档:实收效率是升级频率的实际闸门(见 data/economy.ts 的账),
  // 170 时玩家得为捡钱专程绕航线,漏捡直接吃掉升级次数
  dropMagnetRadius: 240, // 磁吸起吸半径 px;本轮唯一要能"拖一下看体感"的旋钮(验收标准第四条)

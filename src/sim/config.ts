...
export const tuning = {
  ...
  // —— 加速技能(空格,畅玩性)——World.step 每逻辑帧现读,面板拖动即时生效(与船那批同口径)。
  // 触发 = 空格且冷却归零;冷却从触发起算(含加速窗),实际"技能真空期" = cooldown - duration。
  boostSpeedMul: 2.1, // 加速窗内巡航上限倍率(130 → ~273 px/s,略快于最快的敌型);占位待调
  boostAccelMul: 3, // 加速窗内推力倍率:一脚踹到位,而不是按老爬升曲线慢慢磨上去;占位待调
  boostDuration: 1.1, // 加速窗时长(秒):够横穿一次包夹缺口,不够绕场兜圈;占位待调
  boostCooldown: 5, // 冷却(秒,从触发起算):真空期 ~3.9s —— 每波包夹给一次逃生机会;占位待调
  // 加速窗内船体受伤倍率(<1 = 减伤):让"往怪堆缺口里冲"真的是逃生手段而不是自杀 ——
  // 窗内擦着蜂群边缘冲出去只掉七成血。作用点在 World.shipDamageTakenMul(撞击与弹幕都吃)
  boostDamageTakenMul: 0.7,

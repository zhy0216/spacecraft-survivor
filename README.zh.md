...
太空割草(Survivors-like)× 甲板空间拼图(Backpack-like)。
...
```
src/core     循环、种子 RNG、对象池、空间哈希、输入
src/sim      世界状态与规则(纯 TS,无渲染依赖)
src/render   Pixi 渲染、镜头、灰盒纹理
src/ui       DOM 覆盖层:标题、暂停、HUD、升级、整备商店、图鉴、结算、调参面板
src/data     数值配置(塔/敌人/波次/法令/词缀/经济)
src/i18n     语言资源与质量门禁(zh-CN / en、伪语言)
worker/      Cloudflare Worker:POST /api/logs → R2 + D1 配额
...
```bash
npm run balance              # 求解数值表,跑七份裁判测试当验收门
npm run balance -- --dry-run # 只打印求解结果,不落盘、不跑测试
```
